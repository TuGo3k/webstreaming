  ...
  useEffect(() => {
   ...
   return (    
    ...
    <div  className=" w-full fixed top-0 left-0 bg-[#252631] lg:h-[106px] lg:flex justify-evenly items-center py-5 px-[5%] md:px-[9%] z-20">
      ...
      {/* Desktop Search Bar */}
      <div ref={searchRef} className="relative hidden md:flex items-center bg-[#12151e] h-[46px] p-3 rounded-full">
        <input
          type="text"
   ...
   {isMobileMenuOpen && (
    <div  className="MOBILE_SEARCHBAR absolute transition-all duration-1000 ease-in-out left-0 w-full bg-[#252631] text-white flex flex-col gap-4 p-4 z-30 md:hidden">
      ...
      {categories.map((category, index) => (
        ...
        <Link
        to={"/login"} onClick={() => setIsMobileMenuOpen(false)}
        className={` text-[14px] border-2 border-yellow-400 rounded-full px-9 py-3 text-white font-bold bg-[#12151e] hover:bg-[#e4d804] hover:text-black ${invisibleLogin ? "hidden" : "block lg:hidden"}` }
      >
        SIGN IN
      </Link>
      </div>)}
      ...
      <Link
        to={"/login"} onClick={() => setIsMobileMenuOpen(false)}
        className={`${invisibleLogin ? " hidden" : "hidden lg:block " } text-[14px] border-2 border-yellow-400 rounded-full px-9 py-3 text-white font-bold bg-[#12151e] hover:bg-[#e4d804] hover:text-black`}
      >
        SIGN IN
      </Link>
